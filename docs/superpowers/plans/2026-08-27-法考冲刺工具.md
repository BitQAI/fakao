...
def assistant_context(conn, question: str,
    ...
    for e in related:
        ...
        for s in e["sources"]:
            ...
            src_file = config.SOURCE_DIR / ref
            if src_file.exists():
...
Expected: 全绿（smoke 2 + db 4 + importer 6 + scheduler 6 + coverage 3 + ai 8 + tts 4 + service 9 + api 10 = `52 passed`）
...
python3 -m venv .venv
.venv/bin/pip install -r requirements.txt
.venv/bin/python scripts/import_entries.py /path/to/条目库/*.json
```
...
1. **Spec 覆盖度**：spec 3.1 四 Tab（今日/学习/报告/我的）→ Task 11–17；3.2 功能 1 动态倒排 → Task 4/8；2 看背 → Task 12；3 听学 → Task 13；4 自测 → Task 14；5 复盘 → Task 8/15；6 覆盖树 → Task 5/15；7 AI 助手 → Task 6/16；8 数据导入 → Task 3/17；错误处理九 → Task 6 兜底 + Task 19 降级验证；测试十 → Task 19；部署十一 → Task 18。无缺口。
