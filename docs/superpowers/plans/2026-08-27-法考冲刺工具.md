...
- 数据源格式：`fakao-entry/1.0` 侧车 JSON（13 字段，字段名与 spec 第五节一致），导入失败必须拒绝整批写入（不允许半写入）
...
│   └── entries/                      ← Task 0 产出的 13 字段 JSON（入库）
├── .env.example                        ← Task 1 新建（key 占位，真实 .env 不入库）
├── backend/
│   ├── requirements.txt                ← Task 1
│   ├── app/
│   │   ├── __init__.py
│   │   ├── config.py                   路径/环境变量唯一入口        ← Task 1
│   │   ├── db.py                       SQLite 连接 + 8 张表 DDL    ← Task 2
│   │   ├── importer.py                 13 字段 JSON 校验 + 入库    ← Task 3
│   │   ├── scheduler.py                三级调度 + 动态倒排（纯函数） ← Task 4
│   │   ├── coverage.py                 覆盖树聚合（纯函数）         ← Task 5
│   │   ├── ai.py                       DeepSeek 客户端 + 规则兜底  ← Task 6
│   │   ├── tts.py                      edge-tts 合成 + 缓存        ← Task 7
│   │   ├── service.py                  今日计划/统计/自测/复盘服务   ← Task 8
│   │   ├── main.py                     FastAPI 实例 + 挂载路由     ← Task 9
│   │   └── routers/
│   │       ├── __init__.py
│   │       ├── today.py  plans.py  reviews.py  audio.py
│   │       ├── quizzes.py  reports.py  coverage.py
│   │       ├── assistant.py  settings.py  import_.py
│   ├── scripts/import_entries.py       CLI 导入命令                ← Task 3
│   └── tests/
...
│   ├── src/app/layout.tsx  page.tsx  globals.css                  ← Task 10 起
...
│   └── src/lib/api.ts
├── deploy/
│   ├── fakao-backend.service  fakao-frontend.service  nginx.conf   ← Task 18
└── README.md                          ← Task 18（部署 + 使用）
```

---

### Task 0: 种子条目生产（首批 300–400 条，与 Task 1–9 并行）
...
- Test: 复用 Task 3 `importer` 校验（校验步骤在 Task 3 提交后执行）

**Interfaces:**
- Consumes: `data/科目资料/<科目>/*.md`（22 份）、`data/备考研究/`、`data/真题回忆版/`
- Produces: `fakao-entry/1.0` 侧车 JSON（status=draft，人工审核后改 final）；是 Task 3 导入与 Task 19 验收的输入
...
| 10 | sources | ≥1 条，`[{"type":"高频|易错|讲义|新规|真题|研究","ref":"<真实文件名>","loc":"<章节/编号>"}]`；ref 必须在 data/ 树中真实存在 |
...
- [ ] **Step 2: 跑通校验（依赖 Task 3 importer 就绪）**
...
- 表结构（后续任务依赖的列名必须与此一致）：`entries / reviews / quizzes / quiz_answers / daily_plans / reports / chat_logs / settings`
...
def test_connect_creates_tables(tmp_db):
    ...
    assert {"entries", "reviews", "quizzes", "quiz_answers",
            "daily_plans", "reports", "chat_logs", "settings"} <= names
...
def test_entries_columns(tmp_db):
    ...
    assert "id" in cols and "tts_text" in cols and "status" in cols
...
### Task 3: 条目导入与校验（importer.py + CLI）
...
INSERT OR REPLACE INTO entries
 (id, subject, submodule, point, anchor, conclusion, priority, tags,
  rationale, sources, statutes, note, tts_text, status)
VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
"""
...
### Task 9: FastAPI 路由与 API 测试

**Files:**
- Create: `backend/app/routers/__init__.py`
- Create: `backend/app/routers/today.py`、`plans.py`、`reviews.py`、`audio.py`、`quizzes.py`、`reports.py`、`coverage.py`、`assistant.py`、`settings.py`、`import_.py`
...
Expected: 全绿（smoke 2 + db 4 + importer 7 + scheduler 6 + coverage 3 + ai 8 + tts 4 + service 9 + api 10 = `53 passed`）
...
- [ ] **Step 11: 写 `frontend/src/app/page.tsx`（占位，Task 11 替换）**
...
- Produces: 学习页三段切换（看背/听学/自测）；看背卡片（翻面 + 三键自评 + 问 AI 事件）
...
（ListenView/QuizView 在 Task 13/14 创建，先建占位避免构建失败：）

```tsx
// frontend/src/components/ListenView.tsx（Task 13 替换）
...
```tsx
// frontend/src/components/QuizView.tsx（Task 14 替换）
...
### Task 18: 部署（systemd + Nginx + README）
...
- [ ] 在「我的」上传一份真实 13 字段 JSON，提示「导入成功 N 条」
...
见 `docs/superpowers/plans/2026-08-27-法考冲刺工具.md` Task 19：导入 → 今日 → 看背 → 听学 → 自测 → 复盘/覆盖 → AI 助手 → 降级验证。
```
...
1. **Spec 覆盖度**：spec 3.1 四 Tab（今日/学习/报告/我的）→ Task 11–17；3.2 功能 1 动态倒排 → Task 4/8；2 看背 → Task 12；3 听学 → Task 13；4 自测 → Task 14；5 复盘 → Task 8/15；6 覆盖树 → Task 5/15；7 AI 助手 → Task 6/16；8 数据导入 → Task 3/17；spec 2.1 数据分层/清洗策略/首批规模 → Task 0 + Global Constraints；源材料递归检索 → Task 1 config + Task 8；ref 软警告 → Task 3；错误处理九 → Task 6 兜底 + Task 19 降级验证；测试十 → Task 19；部署十一 → Task 18。无缺口。
...
完成后逐任务提交（提交信息已在各任务 Step 末尾给出），最后按 Task 19 全链路验收。
