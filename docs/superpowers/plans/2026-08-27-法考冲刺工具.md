...
- 数据源格式：`fakao-entry/1.0` 侧车 JSON（12+1 字段：12 必填 + `cases` 可选，字段名与 spec 一致），导入失败必须拒绝整批写入（不允许半写入）
...
- 种子数据：首批 300–400 条 final（刑法/民法各 50 先行跑通全链路，再补 5 科高频考点），之后每日增量 50–100 条（易错陷阱优先）；status 由人工审核 draft→final，调度器只消费 final
...
│   └── entries/                      ← Task 1 产出的 12+1 字段 JSON（入库）
...
2. 生产顺序：刑法 → 民法（各 50 条 高频考点先行）→ 刑诉 → 民诉 → 商经知 → 理论法 → 三国法（各 40–60 条，三国法可少）；首批只做 高频考点，易错陷阱留作每日增量（50–100 条/天）。
3. 2026 年新法（刑法修正案十二、新公司法、民诉涉外编、醉驾意见等）标 priority=新增必考，优先收录。

## 12+1 字段规范（顺序固定；案例为可选字段）
| # | 字段 | 要求 |
|---|---|---|
...
| 7 | priority | 单选文本：高频考点 / 易错陷阱 / 新增必考 / 普通 |
| 8 | rationale | 为何值得记，一句话 |
| 9 | sources | ≥1 条，`[{"type":"高频|易错|讲义|新规|真题|研究","ref":"<真实文件名>","loc":"<原文摘录句>"}]`；ref 文件名与 loc 摘录文本都必须能在 data/ 树对应文件中找到（找不到出软警告 W）；**来源类型不含案例**（案例走 13 字段） |
| 10 | statutes | 法条定位数组（如 ["刑法269条", "民法典第1165条"]）；纯理论填 []；点击经 `/api/statute` 弹出条文原文，解析不到显示「原文不可用」 |
| 11 | note | 易错陷阱/关联，≤80 字，可 null |
| 12 | tts_text | 非空，≤150 字：`【{科目}·{考点}】{锚点句}。{结论句}` |
| 13 | cases | 可选，0–3 条，`[{"source":"人民法院案例库|司法部案例库|最高检指导性案例|最高法指导性案例","loc":"case_00001|检例第39号|指导性案例001号"}]`；**loc 必须存在于 data/案例库统一/index.csv**；title 由导入时按索引自动补全（存库含 title）；**生产可留空，由 `scripts/match_cases.py` 程序化补配**（关键词检索 index.csv + 正文核验 jsonl，人工抽查剔除误配；引用上限默认不限，可 `--max-refs` 设置）；宁缺毋滥 |
...
- [ ] **Step 1: 生成首批两个科目（刑法、民法各 50 条 高频考点）**
...
```bash
cd backend && .venv/bin/python scripts/match_cases.py   # 程序化补配 cases（每批生成后必跑）
```
...
Expected: 每个文件 `imported` 为该文件条数、`errors` 为空；`warnings` 中的「来源文件不存在」「摘录文本不存在」「法条无法解析」必须为 0（生产时 ref、loc 摘录、法条全部真实命中）。补配后的 cases 需人工抽查剔除误配后再提交。

- [ ] **Step 3: 补其余 5 科高频**

按 prompt 顺序生成刑诉/民诉/商经知/理论法/三国法，每科 40–60 条 高频考点，重复 Step 2 校验，直至全部 `errors=[]` 且「来源文件不存在」「摘录文本不存在」「法条无法解析」警告为 0。
...
**Files:**
- Create: `backend/app/statutes.py`
- Create: `backend/app/importer.py`
- Create: `backend/scripts/import_entries.py`
- Test: `backend/tests/test_importer.py`

**Interfaces:**
- Consumes: `db.connect`；`app.config.SOURCE_DIR / STATUTE_DIR / CASES_INDEX`
- Produces:
  - `statutes.resolve_statute(text: str) -> str | None`（"刑法269条"等 → 条文正文；法条库运行时解析 + 缓存，供 importer 校验与 `/api/statute` 共用）
...
- [ ] **Step 3: 写 `backend/app/statutes.py` 与 `backend/app/importer.py`**

先写 `backend/app/statutes.py`（法条库运行时解析，importer 校验与 `/api/statute` 共用）：
...
Expected: 打印 `{'imported': 1, 'errors': [], 'warnings': [...]}`（临时目录不含来源与法条库，来源文件/摘录/法条三类软警告属预期；`imported=1`、`errors=[]` 即通过），临时目录随后可删（不影响仓库）。
...
- Produces:
  - `scheduler.PRIORITY_ORDER = {"高频考点": 0, "易错陷阱": 1, "新增必考": 2, "普通": 3}`
  ...
  - `scheduler.build_queue(states: list[EntryState], capacity: int, new_ratio: float = 0.35) -> list[str]`（排序：retry ＞ review ＞ new，同级按 高频考点＞易错陷阱＞新增必考＞普通 再按 id；先装满 retry，再按 new_ratio 分 review/new 配额，单边不足时余量给另一边，超容量截断）
...
- Produces（前端唯一入口，路径全部以 `/api` 开头）：
  ...
  - `GET /api/source?ref=<来源库名|文件名>&loc=<定位符|原文摘录>` → 参考来源/案例原文：ref 为案例库枚举时查统一索引（`{"kind":"case","title","text",...}`），否则递归查 data/ 树，以 loc 摘录文本做子串检索返回命中段落（`{"kind":"file","title","text","offset"}`）；都查不到 404
  - `GET /api/statute?law=<法名简称>&no=<条号>` → `{"law","no","text"}`（法条库运行时解析）；解析不到 404
...
- [ ] 在「我的」上传一份真实 12+1 字段 JSON，提示「导入成功 N 条」
...
1. **Spec 覆盖度**：spec 3.1 四 Tab（今日/学习/报告/我的）→ Task 12–18；3.2 功能 1 动态倒排 → Task 5/9；2 看背 → Task 13；3 听学 → Task 14；4 自测 → Task 15；5 复盘 → Task 9/16；6 覆盖树 → Task 6/16；7 AI 助手 → Task 7/17；8 数据导入 → Task 4/18；9 案例原文查看 → Task 0 + Task 10（`/api/source`）+ Task 13/14（SourceViewer）；10 法条原文查看 → Task 4（statutes 校验）+ Task 10（`/api/statute`）+ Task 13/14（SourceViewer 法条分支）；12+1 字段（cases 可选、title 自动补全）→ Task 1 prompt + Task 3（entries 列）+ Task 4（校验）；spec 2.1 数据分层/清洗策略/首批规模 → Task 1 + Global Constraints；法条库运行时解析 → Task 2 config + Task 4 statutes.py；源材料递归检索 → Task 2 config + Task 9；ref/loc/statute 校验 → Task 4；错误处理九 → Task 7 兜底 + Task 20 降级验证；测试十 → Task 20；部署十一 → Task 19。无缺口。
