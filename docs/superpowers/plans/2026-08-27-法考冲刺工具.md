...
| 13 | cases | 可选，0–3 条，`[{"source":"人民法院案例库|司法部案例库|最高检指导性案例|最高法指导性案例","loc":"case_00001|检例第39号|指导性案例001号"}]`；**loc 必须存在于 data/案例库统一/index.csv**；title 由导入时按索引自动补全（存库含 title），生产只写 source/loc；宁缺毋滥，只配「值得点开看原文」的案例 |
